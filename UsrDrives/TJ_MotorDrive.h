...
typedef struct
{
	...
	uint8_t u8ID;
	tjCmdTy_t stCmdType;
	tjMemoryTable_t stTableIndex;
	uint8_t receive_buf[TJ_DATA_RECV_LEN_MAX];									//完整包
...
typedef struct
{
	...
	uint16_t u16Current_position; // 当前位置
	int8_t u8Tempture;						// 温度
	uint16_t u16Current;					// 电流
...
extern tjComm_t stTjCommAllInfo;									 // 关于电机控制、数据存储的总结构体。

extern HandTargetPosition_t tjTarPosition; // 手部控制结构体
extern tjTransmit_t stTjMotorTransmit; // 电机通信发送结构体

extern tjAnswerFrame_t uart2_rx_buf; // 存放电机解析后的数据
extern tjAnswerFrame_t uart3_rx_buf; // 存放电机解析后的数据
extern tjAnswerFrame_t uart4_rx_buf; // 存放电机解析后的数据
bool decode_mc_ans_frame(tjAnswerFrame_t *tran_handle, tjData *pTjData);
